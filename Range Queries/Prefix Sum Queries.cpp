...
pl query(int node, int b, int e, int l, int r) {
  if (l > e || r < b) return {0, 0};
  if (l <= b && e <= r) return {max_prefix[node], sum[node]};
  int mid, left, right;
  mid = (b + e) / 2;
  left = node * 2;
  right = node * 2 + 1;
  pl l1 = query(left, b, mid, l, r);
  pl r1 = query(right, mid + 1, e, l, r);
  pl ans = {0, 0};
  ans.F = max(l1.F, l1.S + r1.F);
  ans.S = l1.S + r1.S;
  return ans;
}
int main() {
